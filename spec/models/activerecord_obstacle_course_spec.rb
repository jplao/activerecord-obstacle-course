...
describe 'ActiveRecord Obstacle Course' do
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(:order_items).select('users.name', 'count(order_items.id) AS total_item_count').group(:user_id).order(name: :desc)
    # ---------------------------------------------------------------
  ...
  xit '30. returns the names of items that have been ordered without n+1 queries' do
    ...
    # ------------------------------------------------------
    orders = Order.all.items.pluck(:name) # Edit only this line
    # ------------------------------------------------------
